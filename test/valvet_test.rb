...
class ValvetTest < Minitest::Test
  include TestFixtures

  def test_that_it_has_a_version_number
    refute_nil ::Valvet::VERSION
  end
  ...
  def test_raises_without_matching_private_key
    hash = {
      "public_key" => Valvet::PublicKey.new(key: OTHER_PUBLIC_KEY),
      "api_key" => Valvet::Encrypted.new(ciphertext: "z+FYZjebo0i+kqJQw/NbomfFaup1srXqTySBWCf67Qy63NjVBSxpj+AH4kjHaz7iuzn8VJgW")
    }
    store = Valvet.new(hash, private_keys: [PRIVATE_KEY])
    assert_raises(Valvet::Error::NoDecryptorError) { store.api_key }
  end
end
